...
const Dashboard = () => {
    ...
    const [category_6, setCategory_6] = useState<number>()
    const [category_7, setCategory_7] = useState<number>()
    const [category_8, setCategory_8] = useState<number>()
    const [category_9, setCategory_9] = useState<number>()
    const [category_10, setCategory_10] = useState<number>()
    ...
    const info = async () => {
        try {
            ...
            setCategory_6(responseData.amount.category_6)
            setCategory_7(responseData.amount.category_7)
            setCategory_8(responseData.amount.category_8)
            setCategory_9(responseData.amount.category_9)
            setCategory_10(responseData.amount.category_10)
        } catch (err) {
    ...
    useEffect(() => {
        console.log('Updated charge_customers value:', data.charge_customers);
      }, [data.charge_customers]);

    const handleRadioChange = (event: { target: { value: string } }) => {
        if (event.target.value === "true") {
            setData((prevData) => ({ ...prevData, charge_customers: true  }));
        } else if(event.target.value === "false"){
            setData((prevData) => ({ ...prevData, charge_customers: false }));
        }
    };
    ...
    const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        if(category_6 < 99 ){
            ...
            return
        } else if (category_7 < 79 || category_8 < 59 || category_9 < 39 || category_10 < 19){
            ...
            <div className=''>
                <form action="" onSubmit={handleSubmit}>
                    ...
                    <div className='m-5 text-center'>
                        <h1 className='text-3xl font-extrabold'>{data.name}, {data.location} on Dhun Jam</h1>
                    </div>
                    <div className='grid grid-cols-2 items-center justify-center w-[600px] gap-6 text-base'>
                        ...
                        <div className='flex flex-col gap-5 w-[300px] justify-center items-center'>
                            <input className={`border-[1px] w-full h-7 rounded-lg text-center 
                            ${!data.charge_customers ? 'bg-[#C2C2C2] cursor-not-allowed ' : 'bg-black text-white'
                                }`}
                            readOnly={!data.charge_customers}  
                            type="number" 
                            value={category_6}
                        ...
                        <div className='flex flex-row gap-5'>
                            <input type="number" className={`border-[1px] h-7 rounded-lg w-[60px] text-center
                            ${!data.charge_customers ? 'bg-[#C2C2C2] cursor-not-allowed ' : 'bg-black text-white'}`} 
                            readOnly={!data.charge_customers}
                    ...
                    <div className='h-auto m-0 relative'>
                        { data.charge_customers && <BarChartComponent categoryData={{...data, category_6, category_7, category_8, category_9, category_10}}/>}
                    </div>
                    { data.charge_customers && <div className='relative bottom-[300px] right-5 w-[10px] items-left justify-start'>
                    ...
                    <div className='flex flex-col gap-5 w-[600px] justify-center items-center mt-5'>
                        ...
                        <input type="submit" value={"Save"} className={` cursor-pointer p-2 rounded-lg p-3 font-bold w-full 
                             ${!data.charge_customers ? 'bg-[#C2C2C2] cursor-not-allowed ' : 'bg-[#6741D9] text-white'}
                              `} />
                    </div>
                    </div>
                </form>
